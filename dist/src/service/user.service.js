...
class UserService {
    ...
    async registerUser(context, input) {
        if (context.user.type !== client_1.UserTypeChoice.ADMIN) {
            throw new apollo_server_express_1.UserInputError("You are not authorized to perform this action");
        }
        if (input.type === client_1.UserTypeChoice.ADMIN) {
            throw new apollo_server_express_1.UserInputError("You cannot create an admin");
        }
